...
CREATE TABLE IF NOT EXISTS OnceGift
...
class SQLiteStorage():
    ...
    :param filename: SQLite数据库的文件名, 默认是 ``OnceGift.sqlite3``
    """
    def __init__(self, filename='once_gift.sqlite3'):
    ...
    def get(self, id):
        ...
        session_json = self.db.execute(
            "SELECT value FROM OnceGift WHERE id=? LIMIT 1;", (id, )
        ).fetchone()
    ...
    def set(self, id, value):
        ...
        """
        self.db.execute(
            "INSERT OR REPLACE INTO OnceGift (id, value) VALUES (?,?);",
            (id, json_dumps(value))
        )
        self.db.commit()
    ...
    def delete(self, id):
        ...
        :param id: 要删除的数据的 id
        """
        self.db.execute("DELETE FROM OnceGift WHERE id=?;", (id, ))
        self.db.commit()
    def append(self, id, dic: dict):
        """
        根据 id 更新数据。会覆盖原有的数据
        """
        updated_dict = self.get(id)
        updated_dict.update(dic)
        self.set(id, updated_dict)
